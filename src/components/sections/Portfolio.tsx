...
const getTechColor = (tech: string): string => {
  const techColors: Record<string, string> = {
    ...
    'Android Studio': 'bg-application-tag/20 text-application-tag border-application-tag/30',
...
function ProjectCardSkeleton() {
  return (
    <div className="bg-eerie-black-2 border border-jet/50 rounded-xl overflow-hidden animate-pulse">
      <div className="h-48 bg-eerie-black-1"></div>
      <div className="p-4 space-y-3">
        <div className="h-6 bg-jet rounded w-3/4"></div>
        <div className="space-y-2">
          <div className="h-4 bg-jet rounded"></div>
          <div className="h-4 bg-jet rounded w-5/6"></div>
        </div>
        <div className="flex justify-between items-center">
          <div className="flex space-x-2">
            <div className="h-4 w-4 bg-jet rounded"></div>
            <div className="h-4 w-4 bg-jet rounded"></div>
          </div>
          <div className="h-3 bg-jet rounded w-16"></div>
        </div>
      </div>
    </div>
  );
...
export default function Portfolio({ onProjectSelect }: PortfolioProps) {
  ...
  const [sortBy, setSortBy] = useState<'default' | 'featured' | 'newest' | 'alphabetical'>('default');
  ...
  return (
    <div ref={portfolioRef} className="space-y-8" role="main" aria-label="Portfolio section">
      ...
      <div className="space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            {(searchQuery || selectedCategory !== 'all' || sortBy !== 'default') && (
              <button
                ...
                className="text-xs text-orange-yellow-crayola hover:text-vegas-gold transition-colors duration-200 px-2 py-1 rounded-lg hover:bg-orange-yellow-crayola/10"
              >
                Clear filters
              </button>
            )}
        ...
        <div className="relative">
          <input
            ...
            className="w-full p-4 pl-12 bg-eerie-black-2 border border-jet/50 rounded-xl text-white-1 placeholder-white-2/60 focus:border-orange-yellow-crayola focus:outline-none focus:ring-2 focus:ring-orange-yellow-crayola/20 transition-all duration-300 backdrop-blur-sm"
          ...
          {searchQuery && (
            <button
              ...
              className="absolute right-4 top-1/2 transform -translate-y-1/2 text-white-2/60 hover:text-orange-yellow-crayola transition-colors duration-300 p-1 rounded-lg hover:bg-orange-yellow-crayola/10"
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div ref={filterRef} className="relative z-[10001]">
            ...
            >
              <span>
                {projectCategories.find(cat => cat.id === selectedCategory)?.label || 'All Projects'}
              </span>
              <svg
                ref={arrowRef}
                className="w-5 h-5 text-white-2/60 transition-transform duration-300"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
      ...
      {!isLoading && filteredProjects.length === 0 && (
        <div className="text-center py-16 px-4">
          <div className="mb-6">
            <svg className="w-20 h-20 text-white-2/30 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M9.172 16.172a4 4 0 015.656 0M9 12h6m-6-4h6m2 5.291A7.962 7.962 0 0112 15c-2.34 0-4.29-1.2-5.5-2.291M15 3H9v3.4a1.6 1.6 0 003.2 0V3z" />
            </svg>
          </div>
          ...
          <p className="text-white-2 text-base mb-6 max-w-md mx-auto">
            Try adjusting your search terms or selecting a different category to explore more projects.
          </p>
          <button
            onClick={clearFilters}
...
const ProjectCard = React.memo<ProjectCardProps>(function ProjectCard({ project, onClick, index }) {
  ...
  return (
    <div
      ...
      className="group relative bg-eerie-black-2 border border-jet/50 rounded-xl overflow-hidden cursor-pointer transform transition-all duration-300 hover:scale-105 hover:border-orange-yellow-crayola/50 hover:shadow-xl hover:shadow-orange-yellow-crayola/10 focus:scale-105 focus:outline-none focus:ring-2 focus:ring-orange-yellow-crayola/50 backdrop-blur-sm"
      role="button"
      tabIndex={0}
      aria-label={`View details for ${project.title}${hasMultipleImages ? ` - ${allImages.length} images` : ''}`}
    >
      ...
      >
        ...
        {isImageLoading && (
          <div className="absolute inset-0 bg-eerie-black-1 animate-pulse flex items-center justify-center">
            <svg className="w-8 h-8 text-white-2/30 animate-spin" fill="none" viewBox="0 0 24 24">
        ...
        {allImages.length > 0 && allImages[currentImageIndex] && !imageError ? (
          <Image
            ...
            fill
            className={`object-cover transition-all duration-500 ${isImageLoading ? 'opacity-0' : 'opacity-100'} ${hasMultipleImages ? 'cursor-pointer' : ''}`}
          ...
          <div className="w-full h-full bg-eerie-black-1 flex items-center justify-center">
            <div className="text-center">
              <svg className="w-12 h-12 text-white-2/30 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {hasMultipleImages && !isImageLoading && !imageError && (
          <>
            ...
            <button
              onClick={prevImage}
              className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-black/80 active:bg-black/90 text-white p-2 sm:p-2 rounded-full backdrop-blur-sm transition-all duration-300 opacity-0 group-hover:opacity-100 touch:opacity-70 z-10 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-orange-yellow-crayola/50"
            ...
            <button
              onClick={nextImage}
              className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-black/80 active:bg-black/90 text-white p-2 sm:p-2 rounded-full backdrop-blur-sm transition-all duration-300 opacity-0 group-hover:opacity-100 touch:opacity-70 z-10 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-orange-yellow-crayola/50"
            ...
            <div className="absolute top-2 right-2 bg-black/70 text-white px-2.5 py-1 rounded-full text-xs backdrop-blur-sm transition-all duration-300 opacity-0 group-hover:opacity-100 sm:opacity-80 border border-white/20">
              {currentImageIndex + 1} / {allImages.length}
            </div>

            {/* Dot indicators - Enhanced for touch */}
            <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex space-x-1.5 opacity-0 group-hover:opacity-100 sm:opacity-80 transition-all duration-300 p-1 rounded-full bg-black/30 backdrop-blur-sm">
              {allImages.map((_, index) => (
                <button
                  key={index}
                  onClick={(e) => goToImage(index, e)}
                  className={`w-2.5 h-2.5 sm:w-2 sm:h-2 rounded-full transition-all duration-300 focus:outline-none focus:ring-1 focus:ring-orange-yellow-crayola/70 ${
            ...
            <div className="absolute top-2 left-2 bg-black/60 text-white p-1.5 rounded-full backdrop-blur-sm border border-white/20">
            ...
            {isHovered && (
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 bg-black/70 text-white px-3 py-1 rounded-full text-xs backdrop-blur-sm animate-pulse sm:hidden">
                Swipe to navigate
              </div>
            )}
          </>
        ...
        <div className="absolute top-3 left-3">
          <span className="px-3 py-1.5 bg-orange-yellow-crayola/90 text-smoky-black text-xs font-semibold rounded-full backdrop-blur-sm border border-orange-yellow-crayola/20 shadow-lg">
      ...
      </div>        {/* Enhanced Project Info */}
      <div className="p-5 space-y-4">
        <h3 className="text-lg font-semibold text-white-1 truncate group-hover:text-orange-yellow-crayola transition-colors duration-300">
          {project.title}
        </h3>
