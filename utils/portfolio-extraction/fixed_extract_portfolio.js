...
function extractProjects() {
  ...
  // Extract individual project items - handle both with and without SensorTower data
  ...
  while ((match = projectItemRegex.exec(projectListContent)) !== null) {
    ...
    // Check for SensorTower URLs in the opening tag
    ...
    if (titleMatch && imageMatch) {
      ...
      console.log(`📱 Extracted: ${project.title} (${project.category})`);
    }
  }
...
function extractProjectDetails(projects) {
  return projects.map(project => {
    ...
    console.log(`   📊 Gallery: ${gallery.length} images, Responsibilities: ${responsibilities.length}, Achievements: ${achievements.length}`);
...
// Write to JSON file
fs.writeFileSync('./portfolio-data-final.json', JSON.stringify(portfolioData, null, 2));

console.log('\n✨ Data saved to portfolio-data-final.json');
console.log('\n📋 Project List:');
